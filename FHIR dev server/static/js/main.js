document.addEventListener('DOMContentLoaded', () => {
    ...
    const btnGetMedications = document.getElementById('btn-get-medications');
    const btnGetProblems = document.getElementById('btn-get-problems');
    const btnGetAppointments = document.getElementById('btn-get-appointments');
    const btnGetDiagnosticReports = document.getElementById('btn-get-diagnostic-reports');
    const btnGetProcedures = document.getElementById('btn-get-procedures');
    const btnGetLabs = document.getElementById('btn-get-labs');
    ...
    async function fetchPatientSpecificData(patientId, dataType) {
        ...
        patientDataResponseArea.innerHTML = '<span class="text-muted">Loading...</span>';
        ...
        try {
            ...
            // Display raw JSON
            responseArea.textContent = JSON.stringify(data, null, 2); // Use main raw response area for consistency

            if (response.ok) {
                let formattedHtml = `<h4>${dataType.charAt(0).toUpperCase() + dataType.slice(1)} for Patient ${escapeHtml(patientId)}</h4>`;
                
                if (data.message) { // Handle case where no data is found (e.g., {"message": "No medications found..."})
                    formattedHtml += `<p>${escapeHtml(data.message)}</p>`;
                } else if (Array.isArray(data) && data.length > 0) {
                    data.forEach(item => {
                        // Assuming each item in the array *could* be treated as a simplified FHIR resource for formatting
                        // This might require more sophisticated mapping if the backend returns simplified JSON
                        // For now, if the item has a resourceType, we'll try to format it.
                        // Otherwise, we just display its JSON.
                        if (item.resourceType) {
                            formattedHtml += formatFhirResource(item);
                        } else {
                            // If the backend returns a simplified array of objects, just stringify them
                            formattedHtml += `<div class="p-2 border rounded mb-2"><pre>${escapeHtml(JSON.stringify(item, null, 2))}</pre></div>`;
                        }
                    });
                } else if (Object.keys(data).length > 0) {
                    // If it's a single object (e.g., an error or an actual FHIR resource not in an array)
                ...
                patientDataResponseArea.innerHTML = `<p class="text-danger">Error fetching ${dataType}: ${escapeHtml(JSON.stringify(data))}</p>`;
            }

        } catch (error) {
            console.error(`Error fetching ${dataType}:`, error);
            const errorMsg = `Request failed: ${error.message}`;
            patientDataResponseArea.innerHTML = `<p class="text-danger">${errorMsg}</p>`;
        }
    }

    // Event Listeners for Patient Data buttons
    btnGetMedications.addEventListener('click', () => {
        const patientId = patientIdInput.value.trim();
        fetchPatientSpecificData(patientId, 'medications');
    });

    btnGetProblems.addEventListener('click', () => {
        const patientId = patientIdInput.value.trim();
        fetchPatientSpecificData(patientId, 'problems');
    });

    btnGetAppointments.addEventListener('click', () => {
        const patientId = patientIdInput.value.trim();
        fetchPatientSpecificData(patientId, 'appointments');
    });

    btnGetDiagnosticReports.addEventListener('click', () => {
        const patientId = patientIdInput.value.trim();
        fetchPatientSpecificData(patientId, 'diagnostic-reports');
    });

    btnGetProcedures.addEventListener('click', () => {
        const patientId = patientIdInput.value.trim();
        fetchPatientSpecificData(patientId, 'procedures');
    });

    btnGetLabs.addEventListener('click', () => {
        const patientId = patientIdInput.value.trim();
        fetchPatientSpecificData(patientId, 'labs');
    });
